...
import { extractGroupId, normalizeCookie } from './socialListeningScraper';
 ...
 */
function getMbasicUrl(postUrlOrId: string, groupUrlOrId?: string): string {
  ...
  if (/^\d+$/.test(cleaned) && groupUrlOrId) {
    const groupId = extractGroupId(groupUrlOrId);
...
export async function postFacebookComment(
  ...
  const rawCookie = normalizeCookie(cookie);
  ...
  try {
    ...
    if (commentId) {
      ...
      if (replyLink) {
        ...
        const replyPageRes = await axios.get(fullReplyUrl, {
