...
export interface LeadMagnetReportData {
  summary: string;
  seoGrade: string;
 ...
 */
async function ensureFontExists(filename: string, url: string): Promise<string> {
  const uploadsDir = path.join(__dirname, '../../uploads');
  if (!fs.existsSync(uploadsDir)) {
    fs.mkdirSync(uploadsDir, { recursive: true });
  }
  const fontPath = path.join(uploadsDir, filename);
 ...
 */
async function scrapeUrlMetadata(url: string): Promise<{ title: string; desc: string }> {
  try {
    const res = await fetch(url, {
 ...
 */
async function generateAuditReportData(url: string, title: string, desc: string): Promise<LeadMagnetReportData> {
...
Yêu cầu định dạng: Trả về DUY NHẤT một đối tượng JSON hợp lệ (không kèm lời dẫn hay markdown), khớp định dạng sau:
{
  ...
  return parseAiJson<LeadMagnetReportData>(rawText);
}

/**
 * Builds A4 PDF document buffer using PDFKit with custom styles and fonts
 */
async function buildAuditPdf(url: string, report: LeadMagnetReportData): Promise<Buffer> {
  // Download Unicode fonts for Vietnamese accents support
  const fontRegular = await ensureFontExists(
    'Roboto-Regular.ttf',
    'https://github.com/google/fonts/raw/main/apache/roboto/static/Roboto-Regular.ttf'
  );
  const fontBold = await ensureFontExists(
    'Roboto-Bold.ttf',
 ...
 */
export async function generateAndSendLeadReport(
  workspaceId: number,
  targetUrl: string,
  email: string,
  name?: string
