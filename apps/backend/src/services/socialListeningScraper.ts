 ...
 */
export function normalizeCookie(cookieInput: string | null | undefined): string {
 ...
 */
function extractCommentsFromContext(context: string): ScrapedComment[] {
...
export async function scrapeFacebookGroup(
  ...
  if (cookie && (cookie.trim().startsWith('{') || cookie.trim().startsWith('['))) {
    try {
      ...
      if (jsonHeaders) {
        const getHeaderValue = (keyName: string): string => {
        ...
        if (parsedUserAgent) userAgent = parsedUserAgent;

        const parsedSecChUa = getHeaderValue('sec-ch-ua');
        if (parsedSecChUa) secChUa = parsedSecChUa;

        const parsedSecChUaMobile = getHeaderValue('sec-ch-ua-mobile');
        if (parsedSecChUaMobile) secChUaMobile = parsedSecChUaMobile;

        const parsedSecChUaPlatform = getHeaderValue('sec-ch-ua-platform');
        if (parsedSecChUaPlatform) secChUaPlatform = parsedSecChUaPlatform;

        const parsedSecChUaFullVersionList = getHeaderValue('sec-ch-ua-full-version-list');
        if (parsedSecChUaFullVersionList) secChUaFullVersionList = parsedSecChUaFullVersionList;

        const parsedSecChUaPlatformVersion = getHeaderValue('sec-ch-ua-platform-version');
        if (parsedSecChUaPlatformVersion) secChUaPlatformVersion = parsedSecChUaPlatformVersion;
      }
    } catch (e) {
  ...
  try {
    ...
    $('script').each((_, scriptEl) => {
      ...
      if (!jsContent.includes('post_id') && !jsContent.includes('Story')) return;
    ...
    const domPosts = new Map<string, ScrapedPost>();
    $('a').each((_, el) => {
      const href = $(el).attr('href') || '';
      const match = href.match(/\/groups\/[^/]+\/(?:posts|permalink)\/(\d+)/);
      if (match) {
        const postId = match[1];
        if (seenPostIds.has(postId) || domPosts.has(postId)) return;
 ...
 */
export function extractGroupId(urlOrId: string): string {
