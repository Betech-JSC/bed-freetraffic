...
import { retrieveRelevantChunksStructured } from '../lib/embeddings';
 ...
 */
async function getEventContext(payload: any) {
  ...
  if (customerId) {
    ...
    if (customer?.notes) {
      notes = customer.notes.map((n: any) => n.content);
    }
  }
 ...
 */
export async function generateEmailWithRag(
  workspaceId: number,
  eventType: string,
  payload: any
...
Context: ${JSON.stringify(context)}`;
    ...
    try {
      const expansionRes = await fetchWithRetry(ai.url, {
        method: 'POST',
        headers: ai.headers,
        body: JSON.stringify({
          model: ai.model,
      ...
      if (expansionRes.ok) {
        ...
        if (parsed.businessQuery) businessQuery = parsed.businessQuery;
        if (parsed.layoutQuery) layoutQuery = parsed.layoutQuery;
      }
    } catch (err) {
    ...
    const [businessChunks, layoutChunks] = await Promise.all([
      retrieveRelevantChunksStructured(workspaceId, businessQuery, 4),
      retrieveRelevantChunksStructured(workspaceId, layoutQuery, 4)
    ]);
    ...
    if (layoutChunks.length === 0) {
      finalLayoutChunks = await retrieveRelevantChunksStructured(workspaceId, 'Email template.pdf layout HTML', 3);
    }
...
Không kèm bất kỳ text giải thích hay code block markdown \`\`\`json.`;
    ...
    const mainRes = await fetchWithRetry(ai.url, {
      method: 'POST',
      headers: ai.headers,
      body: JSON.stringify({
        model: ai.model,
    ...
    if (mainRes.ok) {
      ...
      const content = data.choices?.[0]?.message?.content || '';
      const result = parseAiJson<EmailGenerationResult>(content);
 ...
 */
export function getFallbackEmail(eventType: string, context: any): EmailGenerationResult {
      ...
      if (context.orderItems && context.orderItems.length > 0) {
        itemsListHtml = `
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            ...
            <tbody>
              ${context.orderItems.map((item: any) => `
