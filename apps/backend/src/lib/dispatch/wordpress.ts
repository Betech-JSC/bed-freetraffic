...
export async function dispatchWordPress(payload: DispatchPayload): Promise<DispatchResult> {
  ...
  if (conn?.accessToken && conn.accessToken.trim().startsWith('{')) {
    try {
      ...
      if (parsed.appPassword) password = parsed.appPassword;
    } catch {
  ...
  try {
    ...
    const postData = (await res.json()) as { link: string };
    return {
      success: true,
