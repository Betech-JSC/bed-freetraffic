...
router.get('/', authenticate, workspaceMiddleware as any, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const widgets = await prisma.marketingWidget.findMany({
...
router.post('/', authenticate, workspaceMiddleware as any, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const widget = await prisma.marketingWidget.create({
      data: {
...
router.put('/:id', authenticate, workspaceMiddleware as any, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { name, title, description, configJson, themeColor, isActive } = req.body;

    const widgetId = parseInt(String(id), 10);
    const existing = await prisma.marketingWidget.findFirst({
    ...
    const updated = await prisma.marketingWidget.update({
      where: { id: widgetId },
      data: {
...
router.delete('/:id', authenticate, workspaceMiddleware as any, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const widgetId = parseInt(String(id), 10);
    ...
    await prisma.marketingWidget.delete({ where: { id: widgetId } });
...
router.post('/generate-quiz', authenticate, workspaceMiddleware as any, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/public/script/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const widget = await prisma.marketingWidget.findUnique({ where: { id } });
...
router.post('/public/lead', async (req: Request, res: Response): Promise<void> => {
  try {
