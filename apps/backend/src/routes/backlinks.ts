...
export async function discoverBacklinksHandler(req: AuthRequest, res: Response): Promise<void> {
  ...
  try {
    const result = await discoverBacklinksFromUrl(targetUrl, scanUrl);
    res.json({
...
router.get('/', async (_req: AuthRequest, res: Response): Promise<void> => {
  const links = await prisma.backlink.findMany({ orderBy: { discoveredAt: 'desc' } });
  res.json(links);
});
...
router.post('/:id/estimate-da', requireWrite, async (req: AuthRequest, res: Response): Promise<void> => {
  const id = parseInt(req.params.id as string);
  const link = await prisma.backlink.findUnique({ where: { id } });
  if (!link) {
  ...
  res.json({
    link: updated,
    estimated: true,
    message: mozConfigured
      ? 'DA ước lượng (Moz credentials có — tích hợp API đầy đủ ở bản sau).'
      : 'DA ước lượng theo TLD — thêm MOZ_ACCESS_ID + MOZ_SECRET_KEY cho DA chính xác.',
  });
});
