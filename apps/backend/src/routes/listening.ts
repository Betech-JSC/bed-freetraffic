...
router.post('/campaigns', requireWrite, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const campaign = await prisma.socialListeningCampaign.create({
    data: {
      ...
      autopilotDelayMin: autopilotDelayMin ? parseInt(autopilotDelayMin as string, 10) : 3,
      autopilotDelayMax: autopilotDelayMax ? parseInt(autopilotDelayMax as string, 10) : 7,
      maxPostAgeHours: maxPostAgeHours ? parseInt(maxPostAgeHours as string, 10) : 0,
      knowledgeSources: knowledgeSourceIds && Array.isArray(knowledgeSourceIds) ? {
        connect: knowledgeSourceIds.map((id: number) => ({ id }))
      } : undefined
    },
    include: { knowledgeSources: true }
  });
...
router.put('/campaigns/:id', requireWrite, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const campaign = await prisma.socialListeningCampaign.update({
    ...
    data: {
      ...
      autopilotDelayMin: autopilotDelayMin === undefined ? existing.autopilotDelayMin : parseInt(autopilotDelayMin as string, 10),
      autopilotDelayMax: autopilotDelayMax === undefined ? existing.autopilotDelayMax : parseInt(autopilotDelayMax as string, 10),
      maxPostAgeHours: maxPostAgeHours === undefined ? existing.maxPostAgeHours : parseInt(maxPostAgeHours as string, 10),
      knowledgeSources: knowledgeSourceIds && Array.isArray(knowledgeSourceIds) ? {
        set: knowledgeSourceIds.map((id: number) => ({ id }))
      } : undefined
    },
    include: { knowledgeSources: true }
  });
 ...
 */
router.post('/logs/:id/toggle-autopilot', requireWrite, async (req: AuthRequest, res: Response): Promise<void> => {
  const logId = parseInt(req.params.id as string, 10);
  ...
  try {
    const updated = await prisma.socialListeningLog.update({
      where: { id: logId },
      data: {
 ...
 */
router.post('/logs/:id/reply', requireWrite, async (req: AuthRequest, res: Response): Promise<void> => {
  const logId = parseInt(req.params.id as string, 10);
