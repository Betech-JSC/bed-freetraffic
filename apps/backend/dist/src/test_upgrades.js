...
async function runTests() {
    ...
    try {
        ...
        // Lấy config CSKH
        let cskhConfig = await prisma_1.default.cskhConfig.findUnique({
            where: { workspaceId: workspace.id }
        });
        if (!cskhConfig) {
            cskhConfig = await prisma_1.default.cskhConfig.create({
                data: {
            ...
            await prisma_1.default.cskhConfig.update({
        ...
        for (const msg of messageSequence) {
            ...
            const result = await (0, cskhService_1.handleVisitorMessage)(workspace.id, sessionId, msg, '127.0.0.1', 'Mozilla/5.0');
        ...
        let abTest = await prisma_1.default.abTest.findFirst({
            where: { workspaceId: workspace.id, status: 'RUNNING' }
        });
        if (!abTest) {
            abTest = await prisma_1.default.abTest.create({
                data: {
            ...
            abTest = await prisma_1.default.abTest.update({
                where: { id: abTest.id },
                data: {
                    impressionsA: 110,
                    clicksA: 5,
