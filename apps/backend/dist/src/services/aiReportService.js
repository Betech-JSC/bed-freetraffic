...
class AiReportService {
    static async generateAnalysis(workspaceId, days) {
        ...
        const trafficStats = snapshots.reduce((acc, s) => {
        ...
        const emailCampaigns = await prisma_1.default.emailCampaign.findMany({
            where: {
                workspaceId,
                createdAt: { gte: since }
            }
        });
        const campaignStats = emailCampaigns.reduce((acc, c) => {
            acc.totalCampaigns++;
...
2. Kiểm tra lại hiệu suất các từ khóa SEO để cải thiện vị trí trung bình hiện tại (${avgPosition !== null ? avgPosition : 'chưa có'}).`
        ...
        try {
            const response = await (0, ai_1.fetchWithRetry)(aiConfig.url, {
                method: 'POST',
            ...
            return {
                ...
                issues: [
                    `Tiến trình AI phân tích gặp lỗi: ${error.message}`
                ],
                recommendations: [
