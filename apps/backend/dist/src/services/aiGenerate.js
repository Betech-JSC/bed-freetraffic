...
exports.optimizeSeoContent = optimizeSeoContent;
...
4. TUYỆT ĐỐI KHÔNG viết lời giải thích, lời dẫn, markdown, hay bất kỳ ký tự nào ngoài đối tượng JSON.`;
    ...
    try {
        ...
        if (fetchRes.ok) {
            ...
            const metaDesc = html.match(/<meta[^>]+(?:name|property)=["'](?:description|og:description)["'][^>]+content=["']([^"']*)["']/i)?.[1]?.trim() || html.match(/<meta[^>]+content=["']([^"']*)["'][^>]+(?:name|property)=["'](?:description|og:description)["']/i)?.[1]?.trim();
            const parts = [];
            if (pageTitle)
                parts.push(`Tiêu đề website: "${pageTitle}"`);
            if (metaDesc)
                parts.push(`Mô tả website: "${metaDesc}"`);
            if (parts.length > 0) {
    ...
    try {
        ...
        if (contentType) {
            try {
                ...
                if (!parsed.title)
                    parsed.title = 'Bài viết tự động';
                if (!parsed.content)
                    parsed.content = 'Khám phá ngay tại {url}';
                return parsed;
            }
            catch (parseErr) {
 ...
 */
function extractTitleContent(raw) {
    if (!raw) {
    ...
    if (titleMatch) {
        ...
        if (truncatedTitleMatch)
            title = truncatedTitleMatch[1];
    }
...
Giọng điệu: ${tone}${ragContextText}`;
    try {
        ...
        const contentText = data.choices?.[0]?.message?.content?.trim() || '[]';
        try {
            return (0, ai_1.parseAiJson)(contentText);
        }
        catch {
