...
exports.syncCustomersToMailchimp = syncCustomersToMailchimp;
exports.sendMailchimpCampaign = sendMailchimpCampaign;
const crypto_1 = __importDefault(require("crypto"));
const prisma_1 = __importDefault(require("../lib/prisma"));
function getMailchimpAuthHeaders(apiKey) {
...
async function getMailchimpLists(workspaceId) {
...
async function syncCustomersToMailchimp(listId, customers, workspaceId) {
    const conn = await prisma_1.default.socialConnection.findFirst({
        where: { platform: 'mailchimp', workspaceId }
    });
    const apiKey = conn?.accessToken || process.env.MAILCHIMP_API_KEY;
    ...
    const syncPromises = customers.map(async (customer) => {
        ...
        const subscriberHash = crypto_1.default.createHash('md5').update(email).digest('hex');
...
async function sendMailchimpCampaign(listId, subject, htmlContent, workspaceId) {
    const conn = await prisma_1.default.socialConnection.findFirst({
        where: { platform: 'mailchimp', workspaceId }
    });
    const apiKey = conn?.accessToken || process.env.MAILCHIMP_API_KEY;
