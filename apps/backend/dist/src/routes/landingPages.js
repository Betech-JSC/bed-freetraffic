...
router.get('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const page = await prisma_1.default.landingPage.findUnique({
            where: { id },
            include: { forms: true },
        });
        if (!page) {
...
router.post('/', auth_1.authenticate, auth_1.requireWrite, async (req, res) => {
    try {
        ...
        const existing = await prisma_1.default.landingPage.findFirst({
            where: { slug },
        });
        if (existing) {
...
router.put('/:id', auth_1.authenticate, auth_1.requireWrite, async (req, res) => {
    try {
        ...
        if (slug && slug !== existing.slug) {
            const slugConflict = await prisma_1.default.landingPage.findFirst({
                where: { slug },
            });
            if (slugConflict) {
        ...
        const updated = await prisma_1.default.landingPage.update({
            where: { id },
            data: {
...
router.delete('/:id', auth_1.authenticate, auth_1.requireWrite, async (req, res) => {
    try {
        ...
        await prisma_1.default.landingPage.delete({
            where: { id },
        });
        res.status(204).send();
    }
...
const getThemedUnsplashImage = (theme, prompt) => {
