...
router.get('/onboard-status', auth_1.authenticate, workspace_1.workspaceMiddleware, async (req, res) => {
    try {
        const ws = await prisma_1.default.workspace.findUnique({
...
router.post('/onboard', auth_1.authenticate, workspace_1.workspaceMiddleware, async (req, res) => {
    try {
        ...
        if (websiteUrl && websiteUrl.trim().startsWith('http')) {
            try {
                ...
                const audit = await (0, seoAuditService_1.runSeoAudit)(websiteUrl);
